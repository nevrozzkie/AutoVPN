...
APK сохраняет собственную техническую версию `0.8.0` независимо от тега всего проекта.
...
python3 openwrt/scripts/prepare-release.py \
  ...
  --package /path/to/autovpn-controller-0.8.0-r1.apk \
  --package /path/to/kmod-amneziawg-0-r1.apk \
  --package /path/to/amneziawg-tools-0-r1.apk \
  --min-free-kib 32768 \
  --min-tmp-kib 65536 \
  --output /safe/path/autovpn-router-v0.8.0
```
