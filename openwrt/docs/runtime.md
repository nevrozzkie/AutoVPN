# VPN runtime, negative VPN и Ping all

Пользовательский сценарий этого выпуска включает базовый Wi-Fi, VPN-сеть `-в`
и negative-VPN сеть `-нв`. Они используют один активный профиль и одну AWG-пару.
Zapret-only и VPN+zapret lanes скрыты из LuCI и не считаются активными;
...
Активная lane `vpn` обслуживает `-в` и `-нв`. Для неё доступен ручной/`auto`
выбор VLESS REALITY, Hysteria2 или опционального kernel AmneziaWG. Меньшее время ответа другого VPN или
...
Выбранный профиль сохраняется в runtime bundle v4 с идентификатором lane и используется после перезагрузки,
...
**Services → AutoVPN → Overview** показывает кнопку **Ping all** для общего
VPN runtime. Она выполняет только ручную диагностику кандидатов.
...
Он стоит перед прямыми исключениями; у AWG
...
Обе управляемые VPN-сети создаются на 2.4 и 5 ГГц installer'ом или подтверждаемым действием
...
| Ресурс | Значение |
| --- | --- |
| Bridge / подсеть | `br-avpn`, `192.168.30.0/24` (`-в`, LAN4); `br-avpnnv`, `192.168.33.0/24` (`-нв`, LAN3) |
| Transparent runtime | общий sing-box runtime: listener `12080` для `-в`, `12082` для `-нв` |
| Маршрутизация `-в` | выбранный VPN по умолчанию, российские/ручные исключения напрямую |
| Маршрутизация `-нв` | выбранные домены/CIDR через VPN, остальное напрямую |
| nft guard | общий fail-closed guard закрывает оба моста при остановке runtime |
| SOCKS для проверки | отдельный localhost port каждого VPN-кандидата |
...
Из `br-avpn` и `br-avpnnv` разрешён только IPv4 соответствующих подсетей в
transparent runtime. IPv6 и пересылка в WAN/LAN блокируются. Доступ к самому роутеру ограничен DHCPv4;
...
для `-в`. Ручные исключения сохраняются, приоритет защитных
