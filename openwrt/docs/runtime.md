# Runtime 0.8: переключение только при отказе и Ping all

Реализованы VLESS REALITY, Hysteria2 и опциональный kernel AmneziaWG. `auto` сохраняет
текущий VPN, пока тот работает. Меньшее время ответа другого VPN или восстановление
прежнего VPN **не вызывают переключения**. URLTest не используется в новых конфигах.
Детали AmneziaWG — [отдельно](amnezia.md); в 0.9 добавлен опциональный
[nfqws2 для внешнего VPN-транспорта](zapret.md).
...
Выбранный профиль сохраняется в runtime bundle v2 и используется после перезагрузки,
обновления подписки и перехода из ручного режима в `auto`, если он ещё доступен в
новой конфигурации. Старые bundle v1 проверяются старым renderer и преобразуются
...
**Services → AutoVPN → Overview → Ping all** выполняет только ручную диагностику.
...
1089 VLESS, 1090 Hysteria2, 1091 AmneziaWG. Их маршруты стоят перед прямыми
исключениями; у AWG свой DNS через AWG. Результаты и счётчики проверок не содержат
