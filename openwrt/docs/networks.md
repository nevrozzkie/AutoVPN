...
| `Общага-в` | `br-avpn`, `192.168.30.1/24` | VPN, без прямого выхода при сбое |
| `Общага-з` | `br-avpndz`, `192.168.31.1/24` | SSID и DHCP выключены, forwarding заблокирован |
| `Общага-вз` | `br-avpnz`, `192.168.32.1/24` | SSID и DHCP выключены, forwarding заблокирован |

Последние две сети — подготовленная конфигурация, **не работающие отдельные режимы**.
Не включать их вручную: независимые VPN/VPN+zapret Auto и Ping all ещё не реализованы.
Существующий [zapret для внешнего VPN-транспорта](zapret.md) относится к общей VPN-сети.
