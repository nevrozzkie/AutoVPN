...
Текущий статус: controller и два пакета AmneziaWG собираются точным SDK
...
доменные/IP-исключения для `x-в`, фоновое обновление и статус в LuCI.
...
В текущем пользовательском режиме активны базовая сеть `x`, VPN-сеть `x-в` и
negative-VPN сеть `x-нв`. LAN4 разделяет `x-в`; LAN3 разделяет `x-нв`. В negative
VPN только явно выбранные IPv4-адреса, CIDR и доменные наборы идут через текущий
VPN-профиль, всё остальное остаётся прямым. Zapret-only и VPN+zapret
сети/контролы скрыты из LuCI и не являются частью активного сценария;
соответствующий backend сохранён для совместимости и миграции.
...
Файл `scripts/install.sh` является шаблоном: без зафиксированных адреса релиза,
хеша manifest и публичного ключа он отказывается запускаться. Готовая команда
берётся из [инструкции установки](docs/installation.md). После установки дальнейшая
настройка выполняется в LuCI. Прошивка OpenWrt и работающее WAN-подключение
...
Он создаёт на обоих диапазонах базовый SSID `x` (существующая LAN/администрирование),
VPN-SSID `x-в` и negative-VPN SSID `x-нв` с одним WPA2-паролем. LAN4 входит
в `x-в` с тем же DHCP и fail-closed маршрутом; LAN3 входит в `x-нв`, где
невыбранный IPv4-трафик остаётся прямым. При сбое общего VPN runtime вся `x-нв`
закрывается, чтобы выбранный трафик не утёк в WAN. LAN1/LAN2 остаются в основной LAN. Диапазон выбирает клиент, активный band
steering не ставится. Legacy SSID `x-з` и `x-вз` могут оставаться в старой
конфигурации, но не являются активными пользовательскими сетями и не управляются
этим LuCI-сценарием.
...
Он умеет безопасно восстановить уже полученный snapshot schema v3 для основной
lane; schema v4 остаётся текущим форматом Router API. Активные `x-в` и `x-нв`
разделяют основной профиль и не требуют auxiliary AmneziaWG.

После установки основной сервис запускает только управляющий loop. До привязки к
сайту активная VPN-сеть остаётся закрытой; прямой base SSID сохраняет доступ к
управлению. Legacy zapret guards не открываются и не участвуют в пользовательском
сценарии.
...
В активном пользовательском сценарии работает одна VPN lane `vpn` для `x-в` с
собственными sing-box/AWG процессами, маршрутами, guard и выбором `auto`/ручного
профиля. `Ping all` проверяет кандидаты HTTPS и не меняет активный выбор. Legacy
`vpn_zapret`/`direct_zapret` backend сохранён, но не показывается и не включается
через текущий LuCI. Подробности, ограничения и первый запуск:
