...
В текущем пользовательском режиме активны только базовая сеть `x` и VPN-сеть
`x-в`. Zapret-only и VPN+zapret сети/контролы скрыты из LuCI и не являются частью
активного сценария; соответствующий backend сохранён для совместимости и миграции.
...
и VPN-SSID `x-в` с одним WPA2-паролем. LAN3 и LAN4 входят в ту же VPN-сеть `x-в`,
с тем же DHCP и fail-closed маршрутом; LAN1/LAN2 остаются в основной LAN. Диапазон выбирает клиент, активный band
