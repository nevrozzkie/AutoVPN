...
return view.extend({
	...
	render: function() {
		...
		option.description = _('Choose the name without suffixes, up to 27 UTF-8 bytes (Cyrillic uses more than one byte per letter). The active managed names are the base SSID and -в for VPN; legacy -з and -вз names are not managed here. The same SSID is used on both enabled bands, so clients choose a radio automatically. Save and apply, then open Networks to create the SSIDs.');
