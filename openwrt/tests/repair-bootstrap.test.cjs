...
function fixture(t, options = {}) {
	...
	const controllerName = 'autovpn-controller-0.14.7-r1.apk';
	...
	const manifest = {
		...
		packages: [
			{ name: 'autovpn-controller', filename: controllerName, sha256: digest(controller), version: '0.14.7-r1' },
	...
	let source = template
		.replace('@AUTOVPN_RELEASE_BASE@', 'https://github.com/example/AutoVPN/releases/download/router-v0.14.7-openwrt-25.12.5-r1')
...
if (name === 'id') output('0');
  ...
  if (args.includes('query') && args.includes('kernel')) {
    ...
    if (env.MOCK_BAD_SIGNATURE === '1') process.exit(1);
		...
		output({info:{name:env.MOCK_APK_NAME || 'autovpn-controller',version:env.MOCK_APK_VERSION || '0.14.7-r1',arch:env.MOCK_APK_ARCH || 'noarch'}});
    ...
    const current = fs.existsSync(env.MOCK_INSTALLED_FILE) ? fs.readFileSync(env.MOCK_INSTALLED_FILE, 'utf8') : (env.MOCK_CURRENT || '0.14.0-r3');
		if (env.MOCK_UNSAFE_PLAN === '1') output('(1/2) Upgrading autovpn-controller (' + current + ' -> 0.14.7-r1)\\n(2/2) Installing surprise (1-r1)');
		else output('(1/1) Upgrading autovpn-controller (' + current + ' -> 0.14.7-r1)');
  } else if (args.includes('add')) {
    if (env.MOCK_COMMIT_FAIL === '1') process.exit(1);
		fs.writeFileSync(env.MOCK_INSTALLED_FILE, '0.14.7-r1');
...
test('repairs only the controller, preserves trust and UCI, then releases both locks before Wi-Fi', t => {
	...
	assert.deepEqual(downloads, [
		...
		'autovpn-controller-0.14.7-r1.apk',
	]);
...
test('safe rerun at the repaired version skips APK mutation and retries Wi-Fi with locks released', t => {
	const f = fixture(t, { receiptVersion: '0.14.0-r1' });
	const result = f.run({ MOCK_CURRENT: '0.14.7-r1' });
	assert.equal(result.status, 0, result.stderr);
