...
У APK своя техническая версия (`0.11.0`); это компонент общего обновления AutoVPN 2.0.

**Статус:** исходники объединены локально. APK ещё не собраны, не проверены на
устройстве и GitHub Release не опубликован — готовой публичной команды для
роутера пока нет. Проектная модель включает независимые VPN и VPN+zapret lane,
отдельную прямую zapret-сеть и четыре WPA2 SSID; это не заявление о прошедшей
hardware-проверке. Подробности: [OpenWrt-компонент](openwrt/README.md).
