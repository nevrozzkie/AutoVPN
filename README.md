...
AutoVPN — программное обеспечение для администрирования VPN-инфраструктуры
на подконтрольных владельцу серверах и роутерах: управление клиентами,
конфигурациями подключений, состоянием сервисов и сетевой маршрутизацией.

## Правовые условия использования и публикации

Используйте проект только для правомерных задач и на оборудовании, которым
вы вправе управлять. Наличие исходного кода или технической возможности
подключения не означает разрешения на действия, запрещённые применимым законом.
Проект не обещает анонимности, отсутствия ответственности или допустимости
доступа к любому интернет-ресурсу.

### Российская Федерация

Ниже приведены отдельные правовые ориентиры, проверенные **6 сентября 2026 года**,
а не заключение о соответствии всего проекта законодательству:

- **Публичные материалы.** Описание действий для доступа к ограниченным
  ресурсам, побуждение к ним или предложение такой возможности могут быть
  основаниями для ограничения доступа к публикации. Критерии введены
  [приказом Роскомнадзора от 08.11.2023 № 168](https://publication.pravo.gov.ru/document/0001202312010036).
  [Разъяснение прокуратуры](https://epp.genproc.gov.ru/ru/proc_74/activity/legal-education/explain/e1901811/)
  также указывает на исключение для научной, научно-технической и статистической
  информации. Самостоятельная надпись «в образовательных целях» не подтверждает
  применимость этого исключения к конкретному README, инструкции или релизу.
- **Реклама.** Часть 18 статьи 14.3 КоАП РФ предусматривает ответственность
  за рекламу программно-аппаратных средств доступа к ресурсам и сетям,
  доступ к которым ограничен. Соответствующая норма введена
  [Федеральным законом от 31.07.2025 № 281-ФЗ](https://www.consultant.ru/document/cons_doc_LAW_511068/3d0cac60971a511280cbba229d9b6329c07731f7/)
  с 1 сентября 2025 года. Не распространяйте материалы проекта как рекламу
  доступа к запрещённым ресурсам. Оценка рекламного характера публикации
  требует анализа её содержания и обстоятельств распространения.
- **Эксплуатация сервиса.** Для владельцев соответствующих средств доступа
  предусмотрены требования [статьи 15.8 Федерального закона № 149-ФЗ](https://www.consultant.ru/document/cons_doc_LAW_61798/1a807328c80a540bd0bb724927d6e774595431dc/)
  и ответственность по статье 13.52 КоАП РФ. Применимость требований и исключений
  нужно оценивать для конкретного развёртывания; слова «личный», «семейный»
  или «корпоративный» сами по себе не устанавливают освобождение от них.
- **Действия пользователя.** Статья 13.53 КоАП РФ устанавливает ответственность
  за умышленный поиск заведомо экстремистских материалов, определённых этой
  статьёй, и получение доступа к ним, в том числе с использованием средств
  доступа к ограниченным ресурсам. Это отдельный состав, а не ответственность
  за любое установленное VPN-приложение.
  [Текст введённых норм КоАП РФ](https://www.consultant.ru/document/cons_doc_LAW_511068/3d0cac60971a511280cbba229d9b6329c07731f7/).

### Границы этой документации

Документация описывает реализованные функции, но не подтверждает правомерность
каждого сценария их применения. В частности, база российских доменов/IP служит
маршрутизации и **не является реестром запрещённых ресурсов или механизмом
исполнения требований Роскомнадзора**. Изменение README не меняет поведение
VPN, zapret, установщиков и клиентских приложений.

Этот раздел не заменяет консультацию юриста и не освобождает участников проекта
от установленных законом обязанностей. Для оценки допустимости публичной
публикации или предоставления сервиса необходима отдельная правовая проверка
всего распространяемого комплекта: исходников, инструкций, скриншотов, описаний
PR и релизов. Дисклеймер не устраняет риски, связанные с их фактическим содержанием.
Перед использованием сверяйте действующие редакции норм и применимые требования.
...
на OpenWrt 25.12.x**. После появления подписанного GitHub Release оно будет
устанавливаться одной командой, а дальнейшая настройка будет выполняться через
...
и installer — из неизменяемого GitHub Release этого репозитория.
...
У APK своя техническая версия (`0.14.0`); это компонент общего обновления AutoVPN 2.0.
...
Модуль привязан к конкретному kernel ABI — одной совпадающей модели роутера
недостаточно. [Протокол сборки и измерение размера](openwrt/docs/awg-sdk-build.md).
Установка на устройство и два одновременных AWG-туннеля ещё не проверены.
GitHub Release с установщиком не опубликован: PR с исходниками сам по себе
не создаёт рабочую публичную команду установки роутера.
...
AutoVPN 2.0**. Для выбранной ревизии 2.0 используйте
[установку из checkout](docs/server-installation.md#новая-установка).
Корневые установщики относятся к сайту, не к OpenWrt.
...
- запускает Xray VLESS, Hysteria2 и AmneziaWG (последний — если пакет доступен
  для версии Ubuntu). Для Hysteria2 есть server config, персональная авторизация
  и подписка, но real-client canary на отдельном устройстве ещё не пройден.
...
Маршрут API остаётся `/api/v2/router`. Legacy `GET /snapshot` сохраняет
`schema_version: 3`; `GET /snapshot/dual` отдаёт schema 4 с двумя lane в одном
атомарном снимке. В оба варианта входит стабильный `router_id`, он входит в ETag и
позволяет OpenWrt сверить локально закреплённую identity с устройством, которому
сервер выдал snapshot.

Новый OpenWrt controller требует уже развёрнутый AutoVPN 2.0 и запрашивает
`GET /snapshot/dual`; при `404` он не пробует legacy `/snapshot`. Полученный schema
v3 он может восстановить только для основной lane, без auxiliary AWG и второй lane.
Поэтому сайт обновляют до установки либо обновления такого controller.
...
В списке `/admin/clients` такой клиент помечается `Роутер: <название>` (или ID,
если название не задано). Его обычные ссылки, ключ и статистика остаются на месте.

### Второй AmneziaWG peer: только Router API

Для `vpn_zapret` у Router есть отдельные keypair, PSK и IPv4 в таблице
`router_amnezia_peers`. Это **не дополнительный клиент**. Новый controller
получает его только через аутентифицированный `GET /api/v2/router/snapshot/dual`
со scope `snapshot:read`; schema 4 включает `protocols.amneziawg_aux` без второй
import-ссылки. Legacy `GET /api/v2/router/amnezia/vpn-zapret` сохранён для
совместимости. Обычные `/snapshot` schema v3, `/sub/{token}`, `/amnezia/{token}`,
`/amnezia-key/{token}` и страница клиента второй профиль не раскрывают.

Legacy auxiliary method возвращает `schema_version: 1`, `router_id`, `lane: "vpn_zapret"`,
`revision`, `snapshot_sha256`, `enabled` и структурированный `profile` (без второй
импортной ссылки `vpn://`). При отключённом AWG возвращаются `enabled: false`,
`profile: null`. Действуют `Cache-Control: private, no-store`, сильный ETag и
`If-None-Match`/304. Идентичность определяется исключительно Bearer credential,
а не переданным пользователем router ID.

Ключи создаются при создании Router, а для существующих устройств — однократно
при инициализации после migration 9. Это изменяет desired VPN revision и требует
обычного применения конфигурации сервера. **API не выдаёт ещё не применённый peer**:
до этого возвращается `409 router_peer_not_applied`. Чтение API само по себе не
генерирует ключи, не запускает deploy/SSH и не публикует desired-состояние.
Оба профиля читаются из immutable applied snapshot. Schema 4 отдаёт их вместе,
поэтому controller не смешивает ключи разных применённых ревизий.

Ротация/отзыв API-токена не меняют AWG peer. Отключение Router закрывает его API сразу;
удаление его второго peer с VPN-сервера происходит после применения новой ревизии.
Обычный клиентский ключ этим действием не отзывается. AWG-статистика обоих peer
суммируется у связанного клиента. Второй профиль остаётся только Router API-данными:
сервер не создаёт для него второй обычный client/subscription URL.
...
- Hysteria2 на настроенном UDP-порту, стартовое значение `8443`, с персональной
  `userpass`-авторизацией клиентов;
...
- отдельный сервис `hysteria-server`, server config и `hy2://`-профиль в
  подписке/sing-box;
...
- status проверяет service/UDP и отдельный tunnel handshake с VPS, но это не
  real-client canary на отдельном устройстве и не доказательство production-готовности.
...
Hysteria2-статистика пока не собирается. Это не отменяет поддержку server
config и подписок, но объём трафика клиента по Hysteria2 в панели не отображается.
...
- Для Hysteria2 ещё нет real-client canary и статистики трафика в панели;
  перед production нужен отдельный тест на целевом клиенте/сети.
