...
У APK своя техническая версия (`0.10.0`); это компонент общего обновления AutoVPN 2.0.
...
пока нет. Добавлен первый этап [zapret2 для внешних VPN-соединений](openwrt/docs/zapret.md);
отдельные zapret-сети и проверка на роутере ещё впереди.
...
`router_amnezia_peers`. Это **не дополнительный клиент**. Профиль выдаётся только
через `GET /api/v2/router/amnezia/vpn-zapret` с Bearer scope `snapshot:read`.
Обычный `/snapshot` schema v3, `/sub/{token}`, `/amnezia/{token}`,
`/amnezia-key/{token}` и страница клиента второй профиль не раскрывают.

Новый метод возвращает `schema_version: 1`, `router_id`, `lane: "vpn_zapret"`,
...
Оба профиля читаются из immutable applied snapshot. Будущий двухтуннельный controller
должен сравнивать `router_id`, `revision` и `snapshot_sha256` двух ответов и повторять
загрузку при несовпадении — смешивать ключи разных применённых ревизий нельзя.
...
суммируется у связанного клиента. Получение второго профиля и два независимых
runtime на OpenWrt пока не подключены; этот этап реализует серверную сторону.
