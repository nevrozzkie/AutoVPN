...
- Порты протоколов по умолчанию: VLESS `8443/tcp`, Hysteria `443/udp`, AmneziaWG `51820/udp`. Если оставить порт протокола пустым, AutoVPN не будет хостить этот протокол и уберёт его из подписок. После изменения портов нажмите `Установить / синхронизировать VPN`.
...
- Xray VLESS REALITY + Vision на настроенном VLESS TCP-порту, по умолчанию `8443`;
- экспериментальный Hysteria2-конфиг на настроенном Hysteria UDP-порту, по умолчанию `443`, но в UI он считается не заведённым;
